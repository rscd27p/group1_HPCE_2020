...
inline __attribute__((always_inline)) void rgb2yuv_main_thread(unsigned char *input_image, unsigned char *output_image, uint32_t iterations);
...
void rgb2yuv(unsigned char * __restrict input_image, unsigned char * __restrict output_image, uint32_t total_bytes){
	...
	// Divide by 4 since we have four workers
	uint32_t iterations_per_worker = total_bytes / 192;
	uint32_t bytes_per_worker = total_bytes >> 2;
	...
	pt1_data.iterations = iterations_per_worker;
	...
	pt2_data.iterations = iterations_per_worker;
	pt2_data.input_image = pt1_data.input_image + bytes_per_worker;
	pt2_data.output_image = pt1_data.output_image + bytes_per_worker;
	...
	pt3_data.iterations = iterations_per_worker;
	pt3_data.input_image = pt2_data.input_image + bytes_per_worker;
	pt3_data.output_image = pt2_data.output_image + bytes_per_worker;
	pthread_create( &thread3, NULL, rgb2yuv_async_thread, (void*) &pt3_data);

	// Fourth thread is actually current thread
	rgb2yuv_main_thread(pt3_data.input_image + bytes_per_worker, pt3_data.output_image + bytes_per_worker, iterations_per_worker);
...
void* rgb2yuv_async_thread(void * data_ptr){
	...
	for(uint32_t it_count=0; it_count < pt_data->iterations; it_count++){
		int16x8_t  offset_128_i = vdupq_n_s16(128);
		// Extract R, G and B onto triple register. Each register has 16, 8-bits registers
		uint8x16x3_t rgb  = vld3q_u8(pt_data->input_image + bytes_count);
		...
		vst3q_u8(pt_data->output_image + bytes_count, yuv);
