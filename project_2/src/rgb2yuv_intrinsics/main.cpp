...
void rgb2yuv(unsigned char * __restrict input_image, unsigned char * __restrict output_image, uint32_t total_bytes){
	// Coefficients for Y calculation. Each register is 64 bytes (8 repeated coefficients)
	uint8x8_t y_r_coeff = vdup_n_u8(66);
	uint8x8_t y_g_coeff = vdup_n_u8(129);
	uint8x8_t y_b_coeff = vdup_n_u8(25);

	// Coefficients for U calculation. Each register is 64 bytes (8 repeated coefficients)
	int16x8_t u_r_coeff = vdupq_n_s16(-38);
	int16x8_t u_g_coeff = vdupq_n_s16(-74);
	int16x8_t u_b_coeff = vdupq_n_s16(112);


	// Temporal registers for calculations. They are 8, 16-bit registers
	uint16x8_t  Y_tmp;
	int16x8_t  U_tmp;
	int16x8_t  V_tmp;
	...
	// Offset constants
	uint16x8_t  offset_128 = vdupq_n_u16(128);
	uint16x8_t  offset_16 = vdupq_n_u16(16);
	...
	for(it_count=0; it_count < iterations; it_count++){
		...
		U_tmp = vmulq_s16(vreinterpretq_s16_u8(rgb.val[0]), u_r_coeff);
		U_tmp = vmlal_s8(U_tmp, rgb.val[1], u_g_coeff);
		U_tmp = vmlal_s8(U_tmp, rgb.val[2], u_b_coeff);
		// Add 128 constant to the 8, 16-bit registers
		U_tmp = vaddq_s16(U_tmp, offset_128);
		// Now shift 8 bits to the right
		U_tmp = vshrq_n_u16(U_tmp, 8);
		// Add 128 constant to the 8, 16-bit registers
		U_tmp = vaddq_s16(U_tmp, offset_128);
		// Now saturate and convert back to 8, 8-bits registers which is final result for U
		yuv.val[1] = vqmovn_s16(U_tmp);
